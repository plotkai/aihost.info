...
import { 
  ...
  Copy, Check, Sparkles, ExternalLink, FileCode, Cpu, BookOpen, MessageSquareCode, Layers
} from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-2xl mb-8 relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex items-start gap-4">
            {/* Avatar Icon */}
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 flex items-center justify-center text-brand-400 font-mono font-bold text-2xl shadow-xl flex-shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {server.verified && (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-brand-500/15 text-brand-400 border border-brand-500/30">
                    <ShieldCheck className="w-3.5 h-3.5" />
                    Verified Server
                  </span>
                )}
                {server.aihostBridgeSupported && (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-sky-500/15 text-sky-300 border border-sky-500/30">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mt-2">
                <span>By <strong className="text-slate-200">{server.company || server.author}</strong></span>
                ...
                <span className="text-slate-700">•</span>
                <span>License: <strong className="text-slate-300">{server.license}</strong></span>
              ...
              <div className="flex flex-wrap gap-1.5 mt-4">
                {server.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs px-2.5 py-0.5 rounded-md bg-slate-900 border border-slate-800 text-slate-400"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <div className="flex flex-row md:flex-col gap-2.5 flex-shrink-0">
            {server.repositoryUrl && (
              <a
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-sm font-semibold text-slate-200 hover:text-white transition-all shadow-sm"
              >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-sky-500/20 to-indigo-500/20 hover:from-sky-500/30 hover:to-indigo-500/30 border border-sky-500/40 text-sm font-semibold text-sky-200 transition-all shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="glass-panel p-6 rounded-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-white">Client Configuration Generator</h2>
              </div>
              <span className="text-xs text-slate-400 font-mono">1-Click Config</span>
            </div>

            {/* Client Tabs */}
            <div className="flex flex-wrap gap-1.5 p-1 bg-slate-950/80 rounded-xl border border-slate-800/80 mb-5">
              {clients.map((client) => (
                <button
                  key={client.id}
            ...
            {server.envVars && server.envVars.length > 0 && (
              <div className="mb-5 p-4 rounded-xl bg-slate-900/90 border border-slate-800 space-y-3">
                ...
                {server.envVars.map((v) => (
                  <div key={v.name} className="space-y-1">
                    <label className="text-[11px] font-mono text-slate-400 flex items-center justify-between">
                      <span>{v.name} {v.required && <strong className="text-amber-400">*</strong>}</span>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-slate-950 border border-slate-700/80 rounded-lg text-xs font-mono text-brand-300 placeholder-slate-600 focus:outline-none focus:border-brand-500"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="relative rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
              <div className="flex items-center justify-between px-4 py-2.5 bg-slate-900/90 border-b border-slate-800">
                ...
                <button
                  onClick={() => handleCopy(clientConfig, 'main-config')}
                  className="flex items-center gap-1.5 text-xs font-mono px-3 py-1 rounded-md bg-brand-500/10 hover:bg-brand-500/20 text-brand-400 border border-brand-500/30 transition-colors"
                >
                  {copiedConfig ? (
                    <>
            ...
            {/* File Path Guide */}
            {filePaths.length > 0 && (
              <div className="mt-4 p-3 rounded-lg bg-slate-900/50 border border-slate-800/80">
                ...
                <div className="space-y-1">
                  {filePaths.map((fp, i) => (
                    <div key={i} className="text-[11px] font-mono text-slate-300 flex items-start gap-2">
                      ...
                      <code className="text-brand-300/90 break-all">{fp.path}</code>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="glass-panel p-6 rounded-2xl space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-white">Quickstart & Installation</h2>
            </div>
              ...
              {server.quickstart.installSteps.map((step, idx) => (
                <div key={idx} className="space-y-2">
                  <div className="text-xs font-bold text-slate-200 flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-slate-800 text-brand-400 font-mono text-[10px] flex items-center justify-center border border-slate-700">
                      {idx + 1}
                    </span>
                  ...
                  {step.code && (
                    <div className="ml-7 relative rounded-lg bg-slate-950 border border-slate-800 p-3 text-xs font-mono text-slate-200 overflow-x-auto">
                      <code>{step.code}</code>
                    </div>
                  )}
                </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="glass-panel p-6 rounded-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-white">Tools & Endpoints</h2>
              </div>
              <span className="text-xs font-mono bg-indigo-500/10 text-indigo-300 px-2 py-0.5 rounded-full border border-indigo-500/20">
            ...
            <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1">
              {server.tools && server.tools.length > 0 ? (
                server.tools.map((tool, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800/80 space-y-1.5 hover:border-indigo-500/30 transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="mt-2 pt-2 border-t border-slate-800/60">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {tool.parameters.map((param, pIdx) => (
                            ...
                            >
                              {param.name}: <span className="text-indigo-400">{param.type}</span>
          ...
          {server.quickstart.examples && server.quickstart.examples.length > 0 && (
            <div className="glass-panel p-6 rounded-2xl">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold text-white">Try in Your Assistant</h2>
              </div>
              <div className="space-y-3">
                {server.quickstart.examples.map((ex, i) => (
                  ...
                  >
                    <div className="text-xs font-semibold text-slate-300">{ex.title}</div>
                    <div className="p-2.5 rounded-lg bg-slate-950 border border-slate-800/80 text-xs text-slate-200 font-mono italic">
                      "{ex.prompt}"
                    </div>
                  </div>
                ))}
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-950/40 via-sky-950/30 to-slate-900 border border-sky-500/30 relative overflow-hidden">
            ...
            <button
              onClick={onOpenWaitlist}
              className="w-full py-2 px-4 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs transition-colors shadow-lg shadow-sky-500/20"
            >
